		...
		namespace Detail
		{
			namespace Collider
			{
				...
				Utility::Nullable<ResultPair> 
					AABBAndOBB::IsCollision(IVolume * _pAABB, IVolume * _pObb)
				{
					auto diagrams = CastToVolume(_pAABB, _pObb);

					auto& AABB = diagrams.first->ScaledBV;
					auto& OBBb = diagrams.second->ScaledBV;

					Common::Diagram::OBB OBBa;
					OBBa.Center = (AABB.Max - AABB.Min)*0.5f;
					OBBa.Half = OBBa.Center;
					OBBa.LocalAxisX = Utility::Vector3::rigth;
					OBBa.LocalAxisY = Utility::Vector3::up;
					OBBa.LocalAxisZ = Utility::Vector3::forward;
